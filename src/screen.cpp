...
void show_screen(void)
{
    ...
    {
        if (right > Level->level_width)
        {
            right = Level->level_width;
        }

        if (bottom > Level->level_length)
        {
            bottom = Level->level_length;
        }
    }
    else
    {
        if (right > COUNTRY_WIDTH)
        {
            right = COUNTRY_WIDTH;
        }

        if (bottom > COUNTRY_LENGTH)
        {
            bottom = COUNTRY_LENGTH;
        }
    }
...
int cinema_getch (void)
{
    ...
    if (EOF == response || ESCAPE == response)
    {
        response = -1;
    }
